    ...
    public class db_Air
    {
            ...
            using (QRESTEntities ctx = new QRESTEntities())
            {
                try
                {
                    var xxx = (from a in ctx.T_QREST_SITES
                            join u in ctx.T_QREST_ORG_USERS on a.ORG_ID equals u.ORG_ID
        ...
        public static T_QREST_SITES GetT_QREST_SITES_ByID(Guid SiteIDX)
        {
            using (QRESTEntities ctx = new QRESTEntities())
            {
                try
                {
                    return (from a in ctx.T_QREST_SITES
        ...
        public static SiteMonitorDisplayType GetT_QREST_MONITORS_ByID(Guid id)
        {
            using (QRESTEntities ctx = new QRESTEntities())
            {
                try
                {
                    return (from a in ctx.T_QREST_MONITORS
                            join r in ctx.T_QREST_REF_PAR_METHODS on a.PAR_METHOD_IDX equals r.PAR_METHOD_IDX
                            join p in ctx.T_QREST_REF_PARAMETERS on r.PAR_CODE equals p.PAR_CODE
            ...
            using (QRESTEntities ctx = new QRESTEntities())
            {
                try
                {
                    var xxx = (from a in ctx.T_QREST_MONITORS
                               join r in ctx.T_QREST_REF_PAR_METHODS on a.PAR_METHOD_IDX equals r.PAR_METHOD_IDX 
                               join p in ctx.T_QREST_REF_PARAMETERS on r.PAR_CODE equals p.PAR_CODE
                               join d in ctx.T_QREST_REF_DURATION on a.DURATION_CODE equals d.DURATION_CODE
                                   into lj from d in lj.DefaultIfEmpty() //left join on duration
                               join f in ctx.T_QREST_REF_COLLECT_FREQ on a.COLLECT_FREQ_CODE equals f.COLLECT_FREQ_CODE
                                   into lj2 from f in lj.DefaultIfEmpty() //left join on coll freq
                               where a.SITE_IDX == SiteIDX
                               orderby a.CREATE_DT
                               select new SiteMonitorDisplayType {
                                   T_QREST_MONITORS = a,
        ...
        public static List<SiteMonitorDisplayType> GetT_QREST_MONITORS_ByUser_OrgID(string OrgID, string UserIDX)
        {
            using (QRESTEntities ctx = new QRESTEntities())
            {
                try
                {
                    var xxx = (from m in ctx.T_QREST_MONITORS
                               join a in ctx.T_QREST_SITES on m.SITE_IDX equals a.SITE_IDX
                               join u in ctx.T_QREST_ORG_USERS on a.ORG_ID equals u.ORG_ID
                               join r in ctx.T_QREST_REF_PAR_METHODS on m.PAR_METHOD_IDX equals r.PAR_METHOD_IDX
                               join p in ctx.T_QREST_REF_PARAMETERS on r.PAR_CODE equals p.PAR_CODE
                               join d in ctx.T_QREST_REF_DURATION on m.DURATION_CODE equals d.DURATION_CODE
                                   into lj from d in lj.DefaultIfEmpty() //left join on duration
                               join f in ctx.T_QREST_REF_COLLECT_FREQ on m.COLLECT_FREQ_CODE equals f.COLLECT_FREQ_CODE
                                   into lj2 from f in lj.DefaultIfEmpty() //left join on coll freq
                               where u.USER_IDX == UserIDX
                               && u.STATUS_IND == "A"
                               && (OrgID != null ? a.ORG_ID == OrgID : true)
